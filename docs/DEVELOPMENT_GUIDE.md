...
#### Почему класс лежит именно в `contracts`

Твоё сомнение здесь очень правильное: если всё подряд складывать в один слой,
архитектура быстро превратится в кашу. Но в этом месте класс как раз лежит в
своём слое.

`EmailRequestedV1` — это не domain-модель, не application DTO и не repository.
Это описание внешнего сообщения, которое прилетает из Kafka. То есть это
контракт между двумя сервисами.

Разделение здесь такое:

| Что | Где лежит | За что отвечает |
| --- | --- | --- |
| `EmailRequestedV1` | `src/interfaces/contracts/v1/email_requested.py` | Форма внешнего Kafka JSON |
| `ProcessNotificationDTO` | `src/application/dto/notification.py` | Входные данные use case внутри приложения |
| Domain-модель | `src/core/domain/...` | Внутренние бизнес-сущности, независимые от Kafka |
| Repository | `src/core/repositories.py` или отдельный модуль repository | Доступ к хранилищу, если появится БД |

Почему не `repository`: repository отвечает за сохранение и получение данных из
хранилища. Kafka-сообщение не является хранилищем и не является способом
получить domain-объект из БД. Поэтому класть `EmailRequestedV1` в repository
было бы смешением ответственности.

Почему не `domain`: в `EmailRequestedV1` есть детали внешнего транспорта:
snake_case JSON-поля, версия `V1`, Pydantic-валидация входящего payload,
`HttpUrl`, `EmailStr`, запрет лишних полей. Domain не должен знать, что данные
прилетели из Kafka и были названы именно `verification_url`.

Почему не application DTO: DTO use case должен описывать то, что нужно
приложению для выполнения действия. Kafka-контракт описывает то, что прислал
другой сервис. Эти формы могут быть похожи, но их лучше не склеивать, потому
что внешний контракт меняется по правилам интеграции, а внутренний DTO — по
правилам твоего приложения.

Важно: `email_requested.py` уже является отдельным файлом для отдельного
контракта. В Python нормально, что один класс живёт в файле, названном по
этому контракту. Чистота здесь достигается не тем, что мы разнесли каждое поле
по разным директориям, а тем, что у каждого слоя своя причина для изменения.

Отдельный файл вроде `src/interfaces/contracts/v1/models.py` здесь не делает
архитектуру чище. Он просто создаёт дополнительный переход:

```text
email_requested.py -> импорт из models.py -> EmailRequestedV1
```

В итоге имя файла говорит «контракт email requested», но сам контракт лежит
где-то в другом общем файле. Когда контрактов станет много, такой `models.py`
быстро превратится в длинный список несвязанных событий.

Для Kafka-событий удобнее правило: один внешний event-контракт — один файл.

```text
contracts/v1/email_requested.py
contracts/v1/password_reset_requested.py
contracts/v1/order_receipt_requested.py
contracts/v1/payment_failed.py
```

Так легче искать контракт, менять версию и удалять устаревшее событие. Если у
`email_requested` появятся маленькие вложенные структуры, которые используются
только этим событием, их тоже можно оставить в этом же файле рядом с основным
классом.

Общий файл стоит заводить только для реально общих вещей:

```text
contracts/v1/base.py        # BaseContract, общие validators, общие настройки
contracts/v1/envelope.py    # общий Kafka envelope, если он есть у всех событий
contracts/v1/types.py       # общие NewType/Enum, если они переиспользуются
```

То есть мы не выносим класс в отдельный общий файл просто потому, что это
класс. Мы выносим только то, что действительно переиспользуется несколькими
контрактами.
...
То есть внешний контракт существует только на границе системы. После mapper
остальная application-логика работает уже с внутренним DTO и не зависит от
формата Kafka-сообщения.
